    ...
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: pad, paddingBottom: insets.bottom + 32 }} showsVerticalScrollIndicator={false}>
          ...
          <SectionHeader label="CHILDREN" color="#9B72FF" bg="#F3EFFE" onAdd={() => setChildModal(true)} styles={styles} />
          ...
          <SectionHeader label="CLASSES & ACTIVITIES" color="#4FA6E8" bg="#EAF3FD" onAdd={() => setActModal(true)} styles={styles} />
          <View style={styles.card}>
          ...
          <SectionHeader label="SCHOOL EVENTS" color="#1F9A5A" bg="#EFFDF6" onAdd={() => setEventModal(true)} styles={styles} />
          <View style={styles.card}>
      ...
      {/* Child Modal */}
      <SheetModal visible={childModal} onClose={() => setChildModal(false)} insets={insets} title="Add Child" gradColors={['#9B72FF', '#7C5CE8']} icon="user" theme={theme} styles={styles}>
        <FLabel styles={styles}>Name *</FLabel>
        <TextInput style={styles.input} placeholder="e.g. Arjun" placeholderTextColor={theme.placeholder} value={childForm.name} onChangeText={v => setChildForm(f => ({ ...f, name: v }))} />
        <View style={styles.rowFields}>
          <View style={{ flex: 1 }}><FLabel styles={styles}>Age</FLabel><TextInput style={styles.input} placeholder="e.g. 8" placeholderTextColor={theme.placeholder} value={childForm.age} onChangeText={v => setChildForm(f => ({ ...f, age: v }))} keyboardType="numeric" /></View>
          <View style={{ width: 12 }} />
          <View style={{ flex: 1 }}><FLabel styles={styles}>Grade</FLabel><TextInput style={styles.input} placeholder="e.g. 3rd" placeholderTextColor={theme.placeholder} value={childForm.grade} onChangeText={v => setChildForm(f => ({ ...f, grade: v }))} /></View>
        </View>
        ...
        <SaveBtn onPress={saveChild} disabled={!childForm.name.trim()} colors={['#9B72FF', '#7C5CE8']} label="Add Child" styles={styles} />
      </SheetModal>

      {/* Activity Modal */}
      <SheetModal visible={actModal} onClose={() => setActModal(false)} insets={insets} title="Add Activity" gradColors={['#4FA6E8', '#2E86C8']} icon="zap" theme={theme} styles={styles}>
        ...
        <TextInput style={styles.input} placeholder="Child's name" placeholderTextColor={theme.placeholder} value={actForm.child} onChangeText={v => setActForm(f => ({ ...f, child: v }))} />
        ...
        <View style={styles.rowFields}>
          <View style={{ flex: 1 }}><FLabel styles={styles}>Time</FLabel><TextInput style={styles.input} placeholder="e.g. 5 PM" placeholderTextColor={theme.placeholder} value={actForm.time} onChangeText={v => setActForm(f => ({ ...f, time: v }))} /></View>
          <View style={{ width: 12 }} />
        ...
        <SaveBtn onPress={saveActivity} disabled={!actForm.name.trim() || !actForm.type} colors={['#4FA6E8', '#2E86C8']} label="Save Activity" styles={styles} />
      </SheetModal>

      {/* Event Modal */}
      <SheetModal visible={eventModal} onClose={() => setEventModal(false)} insets={insets} title="Add School Event" gradColors={['#1F9A5A', '#3CB37A']} icon="calendar" theme={theme} styles={styles}>
