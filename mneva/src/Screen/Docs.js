...
export default function DocsScreen({ navigation }) {
  ...
  const loadFiles = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/gdrive/files?type=docs');
  ...
  useEffect(() => {
    ...
    (async () => {
      const [cachedStatus, cachedFiles] = await Promise.all([
        ...
        peekCachedResponse('/api/gdrive/files?type=docs').catch(() => null),
      ]);
      if (cancelled) return;
  ...
  const handleConnect = async () => {
    try {
      const res = await apiFetch('/api/gdrive/connect?platform=mobile&from=Docs');
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topBarTitle}>Docs</Text>
          <View style={{ width: 40 }} />
        </View>
        <View style={styles.connectScreen}>
          <LinearGradient colors={[ACCENT, '#2A6DD9']} style={styles.connectIcon}>
            <Feather name="file-text" size={32} color="#FFFFFF" />
          </LinearGradient>
          <Text style={styles.connectTitle}>Google Docs</Text>
          <Text style={styles.connectSubtitle}>Connect Google Drive to browse and open your Google Docs.</Text>
          <TouchableOpacity style={styles.connectBtn} onPress={handleConnect}>
            <LinearGradient colors={[ACCENT, '#2A6DD9']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.connectBtnGrad}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.topBarTitle}>Docs</Text>
          <Text style={styles.topBarSub}>{files.length} documents</Text>
        </View>
            ...
            return (
              <TouchableOpacity style={styles.fileRow} onPress={() => item.webViewLink && Linking.openURL(item.webViewLink)} activeOpacity={0.7}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.fileMeta}>Doc · {date}</Text>
                </View>
