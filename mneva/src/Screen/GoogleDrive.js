...
const MIME_META = {
  'application/vnd.google-apps.document':     { icon: 'file-text', color: '#4285F4', label: 'Doc' },
  'application/vnd.google-apps.spreadsheet':  { icon: 'grid',      color: '#0F9D58', label: 'Sheet' },
  'application/vnd.google-apps.presentation': { icon: 'monitor',   color: '#F4B400', label: 'Slide' },
...
export default function GoogleDriveScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    // Also listen for deep link OAuth callback
  ...
  const FILTERS = [
    { key: 'drive',  label: 'All',    icon: 'hard-drive' },
    { key: 'docs',   label: 'Docs',   icon: 'file-text' },
    { key: 'sheets', label: 'Sheets', icon: 'grid' },
    { key: 'slides', label: 'Slides', icon: 'monitor' },
  ];
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        ...
        <View style={styles.connectScreen}>
          <LinearGradient colors={[ACCENT, '#0D47A1']} style={styles.connectIcon}>
            <Feather name="hard-drive" size={32} color="#FFFFFF" />
          </LinearGradient>
          <Text style={styles.connectTitle}>Google Drive</Text>
          <Text style={styles.connectSubtitle}>Connect Drive to browse Docs, Sheets, Slides and all your files.</Text>
          <TouchableOpacity style={styles.connectBtn} onPress={handleConnect}>
            <LinearGradient colors={[ACCENT, '#0D47A1']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.connectBtnGrad}>
              <Feather name="hard-drive" size={18} color="#FFFFFF" />
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.topBarTitle}>Drive</Text>
          <Text style={styles.topBarSub}>{files.length} files</Text>
        </View>
