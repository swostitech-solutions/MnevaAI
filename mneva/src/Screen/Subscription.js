          ...
          return (
            <View key={plan.id} style={[styles.planCard, plan.popular && styles.planCardPopular, isCurrent && styles.planCardCurrent]}>
              ...
              <View style={styles.planTop}>
                ...
                {isCurrent && <View style={styles.currentBadge}><Feather name="check" size={12} color={theme.accent} /><Text style={styles.currentText}>CURRENT</Text></View>}
              </View>
              <View style={styles.priceRow}><Text style={styles.price}>{plan.price}</Text><Text style={styles.period}>{plan.period}</Text></View>

              <View style={styles.details}>
                <Detail styles={styles} theme={theme} icon="grid" label="Domains covered" value={plan.domains} note={plan.domainDetail} />
                <Detail styles={styles} theme={theme} icon="shield" label="Autonomy level" value={plan.autonomy} note={plan.autonomyDetail} />
                <Detail styles={styles} theme={theme} icon="database" label="Memory" value={plan.memory} />
                <Detail styles={styles} theme={theme} icon="zap" label="Also included" value={plan.extras} />
              </View>
...
function Detail({ icon, label, value, note, styles, theme }) {
  return (
    <View style={styles.detailRow}>
      <View style={styles.detailIcon}><Feather name={icon} size={14} color={theme.accent} /></View>
...
const createStyles = (theme) => StyleSheet.create({
  ...
  backButton: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: theme.card },
  headerTitle: { fontSize: 18, fontWeight: '800', color: theme.text },
  content: { paddingHorizontal: 20, alignItems: 'center' },
  heroIcon: { width: 54, height: 54, borderRadius: 18, backgroundColor: theme.accent, alignItems: 'center', justifyContent: 'center', marginTop: 10, marginBottom: 14 },
  title: { fontSize: 24, fontWeight: '800', color: theme.text, textAlign: 'center' },
  subtitle: { fontSize: 13, color: theme.muted, textAlign: 'center', lineHeight: 19, marginTop: 8, marginBottom: 20, maxWidth: 340 },
  planCard: { alignSelf: 'stretch', backgroundColor: theme.card, borderRadius: 18, padding: 18, marginBottom: 14, borderWidth: 1, borderColor: theme.border, overflow: 'hidden' },
  planCardPopular: { borderColor: theme.accent, borderWidth: 2 },
  planCardCurrent: { backgroundColor: theme.isDark ? 'rgba(52,199,123,0.06)' : '#FBFFFC' },
  popularBadge: { position: 'absolute', right: 0, top: 0, backgroundColor: theme.accent, paddingHorizontal: 11, paddingVertical: 6, borderBottomLeftRadius: 11 },
  ...
  planTitleWrap: { flex: 1 }, planName: { color: theme.text, fontSize: 19, fontWeight: '800' },
  planReason: { color: theme.muted, fontSize: 12, marginTop: 4, lineHeight: 17 },
  currentBadge: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: theme.isDark ? 'rgba(52,199,123,0.16)' : '#E8F5EE', borderRadius: 8, paddingHorizontal: 7, paddingVertical: 4, marginLeft: 8, alignSelf: 'flex-start' },
  currentText: { color: theme.accent, fontSize: 9, fontWeight: '800' },
  priceRow: { flexDirection: 'row', alignItems: 'baseline', marginTop: 15 }, price: { fontSize: 29, fontWeight: '800', color: theme.text }, period: { color: theme.muted, fontSize: 12, marginLeft: 4 },
  details: { marginTop: 17, borderTopWidth: 1, borderTopColor: theme.border, paddingTop: 3 },
  detailRow: { flexDirection: 'row', paddingTop: 12 }, detailIcon: { width: 28, height: 28, borderRadius: 9, backgroundColor: theme.isDark ? 'rgba(52,199,123,0.14)' : '#EFFBF4', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  detailCopy: { flex: 1, paddingBottom: 1 }, detailLabel: { color: theme.muted, fontSize: 10.5, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.35 },
  detailValue: { color: theme.text, fontSize: 13, fontWeight: '700', lineHeight: 19, marginTop: 1 }, detailNote: { color: theme.muted, fontSize: 11.5, lineHeight: 16, marginTop: 1 },
  planButton: { backgroundColor: theme.accent, borderRadius: 11, alignItems: 'center', paddingVertical: 13, marginTop: 18 }, planButtonCurrent: { backgroundColor: theme.isDark ? 'rgba(52,199,123,0.16)' : '#E8F5EE' },
  planButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '800' }, planButtonTextCurrent: { color: theme.accent },
  securityNote: { flexDirection: 'row', gap: 7, alignItems: 'flex-start', marginTop: 4, paddingHorizontal: 10 }, securityText: { flex: 1, color: theme.muted, fontSize: 10.5, lineHeight: 15 },
});
