...
export default function SlidesScreen({ navigation }) {
  ...
  const loadFiles = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch('/api/gdrive/files?type=slides');
  ...
  useEffect(() => {
    ...
    (async () => {
      const [statusRes, filesRes] = await Promise.all([
        ...
        peekCachedResponse('/api/gdrive/files?type=slides'),
      ]);
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topBarTitle}>Slides</Text>
          <View style={{ width: 40 }} />
        </View>
        <View style={styles.connectScreen}>
          <LinearGradient colors={[ACCENT, '#D49800']} style={styles.connectIcon}>
            <Feather name="monitor" size={32} color="#FFFFFF" />
          </LinearGradient>
          <Text style={styles.connectTitle}>Google Slides</Text>
          <Text style={styles.connectSubtitle}>Connect Google Drive to browse and open your Google Slides presentations.</Text>
          <TouchableOpacity style={styles.connectBtn} onPress={handleConnect}>
            <LinearGradient colors={[ACCENT, '#D49800']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.connectBtnGrad}>
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
      <View style={styles.topBar}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.topBarTitle}>Slides</Text>
          <Text style={styles.topBarSub}>{files.length} presentations</Text>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Feather name="monitor" size={32} color={theme.disabled} />
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity style={styles.fileRow} onPress={() => item.webViewLink && Linking.openURL(item.webViewLink)} activeOpacity={0.7}>
                <View style={[styles.fileIcon, { backgroundColor: theme.isDark ? `${ACCENT}45` : `${ACCENT}30` }]}>
                  <Feather name="monitor" size={20} color={ACCENT} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.fileName} numberOfLines={1}>{item.name}</Text>
                  <Text style={styles.fileMeta}>Slide · {date}</Text>
                </View>
