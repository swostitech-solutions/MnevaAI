...
//   const date = new Date(value);
//   return (
//     !Number.isNaN(date.getTime()) && date >= todayStart && date <= todayEnd
//   );
// }
...
//   return (
//     <View
//       style={{ flexDirection: "row", alignItems: "center", gap: 3, height: 36 }}
//     >
//       {anims.map((anim, i) => (
//         <Animated.View
//           key={i}
...
//       if (!status.granted) {
//         setPhaseSync("error");
//         setReply("Microphone permission denied.");
//         return;
//       }
//       await audioRecorder.prepareToRecordAsync();
//       audioRecorder.record();
...
//     } catch {
//       setPhaseSync("error");
//       setReply("Could not start recording.");
//     }
//   };
...
//       if (!text) {
//         setPhaseSync("error");
//         setReply("Could not understand. Try again.");
//         return;
//       }
//       setTranscript(text);
...
//       // send full history to AI — same as AskAI screen
//       const aRes = await apiFetch("/api/agent/chat", {
//         method: "POST",
//         body: { messages: historyRef.current },
//       });
//       const aiText =
//         aRes.response || aRes.reply || aRes.message || aRes.content || "Done.";
...
//       const cleanReply = aiText
//         .replace(
//           /[\u{1F000}-\u{1FFFF}\u{2600}-\u{27BF}\u{2300}-\u{23FF}\u{2B00}-\u{2BFF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA9F}\u{FE00}-\u{FEFF}]/gu,
//           "",
//         )
//         .replace(/[*_`#~>|\-=+\[\]{}\\^]/g, "")
//         .replace(/https?:\/\/\S+/g, "")
...
//   return (
//     <Animated.View
//       style={[styles.orbOverlay, { opacity: fadeAnim }]}
//       pointerEvents="box-none"
//     >
//       {/* ── Top bar ── */}
//       <Animated.View
//         style={[styles.orbTopBar, { transform: [{ translateY: slideAnim }] }]}
//       >
//         <View style={styles.orbBrand}>
//           <View style={[styles.orbBrandDot, { backgroundColor: p.color }]} />
//           <Text style={styles.orbBrandText}>MNEVA AI</Text>
...
//       {/* ── Centre stage ── */}
//       <Animated.View
//         style={[styles.orbStage, { transform: [{ scale: orbScale }] }]}
//       >
//         {/* Ripple rings */}
//         {(phase === "listening" || phase === "speaking") &&
//           [
...
//               style={[
//                 styles.orbRipple,
//                 { borderColor: p.color, transform: [{ scale }], opacity },
//               ]}
//             />
//           ))}
...
//           style={[
//             styles.orbGlow,
//             { backgroundColor: p.glow, opacity: glowAnim },
//           ]}
//         />
...
//           style={[
//             styles.orbMainBtn,
//             { backgroundColor: p.color, shadowColor: p.color },
//           ]}
...
//             <View style={styles.orbThinkRow}>
//               {[0, 1, 2].map((i) => (
//                 <View key={i} style={styles.orbThinkDot} />
//               ))}
//             </View>
//           ) : (
//             <Feather name={p.icon} size={34} color="#FFFFFF" />
...
//       <Animated.View
//         style={[
//           styles.orbStatusBlock,
//           { transform: [{ translateY: slideAnim }] },
//         ]}
//       >
//         <Text style={[styles.orbPhaseLabel, { color: p.color }]}>
//           {p.label}
//         </Text>
//         <Text style={styles.orbPhaseSub}>{p.sub}</Text>
...
//               <View style={{ flex: 1 }}>
//                 <Text style={styles.orbConvoWho}>YOU</Text>
//                 <Text style={styles.orbConvoText} numberOfLines={2}>
//                   {transcript}
//                 </Text>
//               </View>
//             </View>
...
//             <View style={styles.orbConvoRow}>
//               <View
//                 style={[styles.orbConvoYouDot, { backgroundColor: p.color }]}
//               />
//               <View style={{ flex: 1 }}>
//                 <Text style={[styles.orbConvoWho, { color: p.color }]}>
//                   MNEVA
//                 </Text>
//                 <Text style={styles.orbConvoText} numberOfLines={5}>
//                   {reply}
//                 </Text>
//               </View>
//             </View>
...
//   // FIX: distinguish "genuinely no data" from "the request never reached
//   // the server" (flaky Wi-Fi/cellular handoff, DNS hiccup, etc). Previously
//   // a failed Promise.allSettled call just left state at its default zero
//   // value with no visible signal — a blank/empty account looked identical
...
//         doneMeetingsRes.status === "fulfilled" ? doneMeetingsRes.value : null;

//       // FIX: if every critical call failed together, this is almost never
//       // "the account has no data" — it's a connectivity/backend failure.
//       // Surface it instead of quietly rendering zeros.
//       const criticalResults = [meRes, briefRes, profileRes, tasksRes];
...
//   // FIX: self-heal from a silent connectivity failure. Previously, if every
//   // request failed to even reach the server (flaky Wi-Fi/cellular handoff,
//   // DNS hiccup), the screen was left blank with zero retry until the user
//   // manually backgrounded/foregrounded the app. Now: while loadFailed is
//   // true, retry quietly every 8s in the background until it succeeds.
//   useEffect(() => {
//     if (!loadFailed) return;
//     const retryTimer = setInterval(() => {
//       if (!/inactive|background/.test(AppState.currentState || "")) {
//         loadData(true);
//       }
//     }, 8000);
...
//           if (calendarRes.status === "fulfilled") {
//             const items = Array.isArray(calendarRes.value)
//               ? calendarRes.value
...
//           if (tasksRes.status === "fulfilled") {
//             const allTasks = Array.isArray(tasksRes.value)
//               ? tasksRes.value
//               : [];
//             setAllTasks(allTasks);
//             const STRIPE_COLORS = [
...
//             color,
//             title: task.title,
//             subtitle: task.description || "Pending · AI tracked",
//             isLocal: false,
//           },
//           ...prev,
//         ];
...
//         );
//         setPhoneAlerts(
//           unread.filter((n) => n.source === "android").slice(0, 3),
//         );
//         setUnreadCount(notifs.unreadCount || 0);
//       } catch {}
//     }, 30000);
...
//         if (ledgerData.status === "fulfilled") {
//           const seen = new Set();
//           const pending = (ledgerData.value.entries || []).filter(
//             (e) =>
//               e.status === "pending_approval" &&
//               !seen.has(e.id) &&
//               seen.add(e.id),
...
//         // Keep localPriorities in sync with DB on every poll cycle
//         if (tasksData.status === "fulfilled") {
//           const allTasks = Array.isArray(tasksData.value)
//             ? tasksData.value
//             : [];
//           setAllTasks(allTasks);
//           const STRIPE_COLORS = [
...
//           ? {
//               ...prev,
//               suggestedMeetings: (prev.suggestedMeetings || []).filter(
//                 (m) => m.emailId !== emailId,
//               ),
//             }
//           : prev,
...
//         return `Pay ₹${inp.amount || ""} to ${inp.payee || inp.to || "recipient"}`;
//       case "book_cab":
//         return `Cab: ${inp.pickup || "pickup"} → ${inp.destination || "destination"}`;
//       case "order_food":
//         return `Order from ${inp.restaurant || "restaurant"}`;
//       case "send_email":
//         return `Email to ${inp.recipient || inp.to || "recipient"}`;
//       case "draft_reply":
//         return `Reply draft for ${inp.subject || "email"}`;
//       case "schedule_event":
//         return `Schedule: ${inp.title || "event"}`;
//       case "set_reminder":
//         return inp.message || "Set reminder";
//       default:
//         return action.tool
//           .replace(/_/g, " ")
...
//       setLocalPriorities((prev) =>
//         prev.map((p) =>
//           p.id === tempId ? { ...p, id: task.id, isLocal: false } : p,
//         ),
//       );
//     } catch {
//       // keep optimistic item even if save fails
...
//         [...localPriorities, ...backendTasks].map((p) =>
...
//   const todayEnd = new Date();
//   todayEnd.setHours(23, 59, 59, 999);
//   const todayReminders = calendarItems.filter(
//     (item) =>
//       !isJoinableMeetingLink(item.meetLink) &&
//       item.start &&
//       isToday(item.start, todayStart, todayEnd),
...
//     if (
//       task.status !== "PENDING" ||
//       !isToday(task.createdAt, todayStart, todayEnd) ||
//       /^Meeting · /i.test(task.description || "")
//     )
//       return false;
//     // Do not show legacy phone-alert tasks in Today's Priorities. New phone
//     // alerts no longer create tasks at all (see deviceNotifications route).
//     if (
//       /^Important .+ alert · .+ · priority:\d+$/i.test(task.description || "")
//     )
//       return false;
//     const isReminderTask = /^Reminder ·/i.test(task.description || "");
//     return (
...
//       .replace(/^Reminder set:\s*/i, "")
//       .trim()
//       .toLowerCase();
//     const reminder = calendarItems.find(
//       (item) =>
//         !isJoinableMeetingLink(item.meetLink) &&
//         (item.title || "").trim().toLowerCase() === reminderTitle &&
//         item.start,
...
//     brief?.importantNotifications?.urgentCount || 0;
//   const firstName = user?.name?.trim()?.split(" ")[0] || "there";
//   const importantNotificationHeadline = importantNotificationCount
//     ? `Hey ${firstName} — you have ${importantNotificationCount} important notification${importantNotificationCount === 1 ? "" : "s"}${urgentNotificationCount ? `, including ${urgentNotificationCount} urgent` : ""}.`
//     : null;
...
//         }
//       >
//         {/* FIX: visible connectivity-failure banner. Previously a failed
//             fetch and a genuinely-empty account looked identical (both showed
//             0%/0 tasks with no explanation). This makes the failure visible
//             and gives an explicit retry, instead of silently pretending
//             there's no data. */}
//         {loadFailed && (
//           <TouchableOpacity
...
//                   <Text style={styles.phoneAlertsCountText}>
//                     {phoneAlerts.length}
//                   </Text>
//                 </View>
//               </View>
//               <TouchableOpacity
//                 onPress={() => navigation?.navigate?.("PhoneAlerts")}
//               >
//                 <Text style={styles.viewAllText}>View all</Text>
//               </TouchableOpacity>
//             </View>
...
//                 <View
//                   style={[
//                     styles.phoneAlertPriority,
//                     alert.priority >= 85 && styles.phoneAlertPriorityUrgent,
//                   ]}
//                 >
//                   <Feather
...
//                 </View>
//                 <View style={styles.phoneAlertTextWrap}>
//                   <Text style={styles.phoneAlertTitle} numberOfLines={1}>
//                     {alert.title || alert.appName || "Phone alert"}
//                   </Text>
//                   <Text style={styles.phoneAlertBody} numberOfLines={2}>
//                     {alert.body}
//                   </Text>
//                   <Text style={styles.phoneAlertMeta}>
//                     {alert.appName || "Android"}
//                     {alert.priority >= 60
...
//         {/* Recent logs */}
//         <Text
//           style={[styles.sectionHeader, { marginTop: 8, marginBottom: 10 }]}
//         >
//           RECENT LOGS
//         </Text>
...
//                 <View
//                   style={[
//                     styles.logSkeletonLine,
//                     { width: "45%", marginTop: 6 },
//                   ]}
//                 />
//               </View>
//             </View>
...
//             // and use the actual log timestamp below for one correct AM/PM.
//             const logDetail =
//               log.tool === "set_reminder"
//                 ? rawLogDetail
//                     .replace(/\s*·\s*\d{1,2}:\d{2}\s*(?:AM|PM)\b/gi, "")
//                     .trim()
//                 : rawLogDetail;
//             const logTime =
//               log.tool === "set_reminder"
//                 ? formatPriorityReminderTime(
//                     getReminderScheduledAt(log.title, log.scheduledAt),
//                     log.time,
//                   )
//                 : formatRecentLogTime(log.timestamp, log.time);
//             return (
...
//                 <Text style={styles.logoutTitle}>Sign out?</Text>
//                 <Text style={styles.logoutSubtitle}>
//                   Your session will end and you'll need to sign in again to
//                   access Mneva AI.
//                 </Text>
//                 <TouchableOpacity
...
export default function Home({ navigation }) {
  ...
  const loadData = async (isRefresh = false) => {
    ...
    try {
      ...
      // Keep rendering the last usable screen during a later outage. A failed
      // refresh must not turn a previously loaded account into an empty one.
      // A later outage is still a real connectivity failure. Keep the last
      // good data on screen, but expose the retry state and let the existing
      // 8-second recovery loop repair it automatically.
