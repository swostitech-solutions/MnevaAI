...
export async function fetchHeartRate(auth, startMs, endMs) {
  ...
  const points = (data.bucket || []).flatMap(
    (b) => b.dataset?.[0]?.point || [],
  );
  if (!points.length) return null;
  const vals = points.map((p) => p.value?.[0]?.fpVal || 0).filter(Boolean);
...
export async function fetchSleep(auth, startMs, endMs) {
  try {
    ...
    const totalMs = sessions.reduce(
      (sum, s) => sum + (Number(s.endTimeMillis) - Number(s.startTimeMillis)),
      0,
    );
    return totalMs > 0 ? Math.round((totalMs / 3600000) * 10) / 10 : null;
  } catch {
...
export async function getHealthData(user) {
  const prefs = user?.preferences || {};
  const fitConnected =
    !prefs.googleFit?.disconnected &&
    !!(
      prefs.googleFit?.tokens?.refresh_token ||
      prefs.googleFit?.tokens?.access_token
    );
  const synced = prefs.healthSync || null;
  const today = new Date().toISOString().slice(0, 10);
  const syncedToday = synced?.date === today;
  ...
  if (fitConnected) {
    try {
      ...
      if (auth) {
        ...
        return {
          ...
          sleep: sl
            ? {
                value: sl,
                quality: sl >= 7 ? "Good" : sl >= 5 ? "Fair" : "Poor",
                unit: "hrs",
              }
            : null,
          calories: cal ? { consumed: cal, goal: 2000, unit: "kcal" } : null,
      ...
      logger.warn(
        "Google Fit API fetch failed, falling through:",
        fitErr.message,
      );
    }
    // Fit is connected but auth client unavailable — return empty shell so UI shows connected state
  ...
  if (synced) {
    ...
    return {
      period: "today",
      lastUpdated: synced.lastSynced || new Date().toISOString(),
      source: synced.source || "manual",
      heartRate: hr
        ? {
            value: hr,
      ...
      steps: {
        value: steps,
        goal: stepGoal,
        pct: Math.min(100, Math.round((steps / stepGoal) * 100)),
      },
      sleep: sl
        ? {
            value: sl,
